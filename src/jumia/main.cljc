(ns jumia.main
  "Kotodama WASM entrypoint for the Jumia clean-room actor (L4) — Clojure port.
  ...
  facts (`jumia.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "Product"   :plural "products"   :id-prefix "jumia_pro"
   ...
   {:entity "Customer"  :plural "customers"  :id-prefix "jumia_cus"
    :fields [:email :firstName :lastName :phone] :required [:email :firstName]
    :coerce {} :refs {}}
   {:entity "Collection" :plural "collections" :id-prefix "jumia_col"
...
(defn emit-facts
  "EAVT facts for one record: {\"jumia.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "jumia-compat" :tier tier :entities entities} 200])
